...
#   mkdir -p logs
#   sbatch pre-train/run.sh
#
# Optional overrides:
#   RUN_CONFIG=pre-train/configs/train_200m_fineweb_edu_debug.yml sbatch pre-train/run.sh
#   NPROC_PER_NODE=2 sbatch pre-train/run.sh
#   TOKENIZE_DATASET_WORKERS=8 sbatch pre-train/run.sh
#   LOG_EVERY_STEPS=5 sbatch pre-train/run.sh
...
run_stage "dataset tokenization" \
  "$PYTHON_BIN" pre-train/scripts/tokenize_dataset.py --run-config "$RUN_CONFIG"
...
run_stage "evaluation" \
  "$PYTHON_BIN" benchmarks/scripts/evaluate.py --run-config "$RUN_CONFIG"
