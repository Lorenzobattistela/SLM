...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to a data config under pre-train/configs/data/",
    )
    parser.add_argument(
        "--force",
        action="store_true",
...
def main() -> None:
    ...
    train_packer = TokenPacker(
        train_dir, prefix="train", shard_tokens=int(config["storage"]["shard_tokens"])
    )
    val_packer = TokenPacker(
        val_dir, prefix="val", shard_tokens=int(config["storage"]["shard_tokens"])
    )
