    ...
    if metadata_path.exists():
        with metadata_path.open("r", encoding="utf-8") as handle:
            try:
                ...
                metadata = {}
        dtype = metadata.get("storage_dtype", "uint16")
    else:
        dtype = "uint16"
...
def _run_sft_training(config: dict[str, Any], state: DistributedState) -> None:
    ...
    # Load weights from mid-training checkpoint (loading weights only, discarding optimizer state)
    ...
    if resume_from:
        ...
        if checkpoint_path.exists():
            ...
            _log_main(
                state,
                "Initialized weights from checkpoint: %s (optimizer/scheduler state discarded)",
                checkpoint_path,
            )
        else:
            _log_main(
                state,
                "Checkpoint not found at %s. Starting with random weights.",
                checkpoint_path,
            )
    ...
    _log_main(
        state,
        "Starting SFT training project=%s device=%s precision=%s ddp=%s world_size=%s "
    ...
    try:
        ...
        if state.is_main_process:
            ...
            LOGGER.info("SFT training complete at step=%s tokens_seen=%s", step, tokens_seen)

            # Generate SFT plots if enabled
            if bool(config.get("plots", {}).get("enabled", True)):
                try:
                    ...
                    LOGGER.info("Saved SFT training plots to %s", plots_output_dir)
...
def run_sft_training(config: dict[str, Any]) -> None:
    ...
    try:
        _run_sft_training(config, state)
    finally:
        cleanup_distributed(state)
